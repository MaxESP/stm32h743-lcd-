...
uint32_t QSPI_ReadID(void) {
    ...
    // Configure the command for reading the ID
    sCommand.InstructionMode = QSPI_INSTRUCTION_1_LINE;
    ...
    sCommand.DataMode = QSPI_DATA_1_LINE;
    sCommand.DummyCycles = 0;
    sCommand.NbData = 3; // Usually, the ID is 3 bytes long
    ...
    if (HAL_QSPI_Receive(&hqspi, id, HAL_QSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
        ...
        // Combine the ID bytes into a single 32-bit value
        uint32_t flash_id = (id[0] << 16) | (id[1] << 8) | id[2];
        return flash_id;
    }
/* USER CODE END 1 */
